<script lang="tsx">
  ...
  import { tableTitle, options, dialogSchemas, pressionList, permissionId } from './RoleConfig';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      let pList = ref<TreeItem[]>([]);

      async function getPressionList() {
        pList.value = await pressionList();
      }
      ...
      return () => (
        <PageWrapper>
          ...
          {visible.value ? (
            <MFormDialogView
              visible={visible}
              schemas={dialogSchemas(itemInfo.value, pList)}
              title={'新增/编辑角色'}
              onSubmit={onSubmit}
            />
          ) : null}
      ...
      async function onSubmit(params) {
        ...
        if (unref(itemInfo).id == null) {
          value = await addRole(params);
        } else {
          value = await editRole(params);
        }
      ...
      function initData() {
        return getTableData(async () => {
          const value = await getRoleList(params);
          if (value == null || !value.success || value.result == null) {
            return;
          }
      ...
      async function editItem(item: any) {
        ...
        item.permissionIds = await permissionId(item.id);
        itemInfo.value = item;
      }

      //删除信息
      async function delItem(item: any) {
        let value = await delRole({ roleIds: item.id });
